...
private:
	...
	*/
	...
	void drawTriangle(Particle *p1, Particle *p2, Particle *p3, const Vec3 color)
	{
        ...
        Vec3 t1 = p1->getNormal().normalized();
		glNormal3fv((GLfloat *) &(t1) );
		glVertex3fv((GLfloat *) &(p1->getPos() ));

        Vec3 t2 = p2->getNormal().normalized();
		glNormal3fv((GLfloat *) &(t2) );
		glVertex3fv((GLfloat *) &(p2->getPos() ));

        Vec3 t3 = p3->getNormal().normalized();
		glNormal3fv((GLfloat *) &(t3) );
...
//				std::cout<<pos.f[0] << " " << pos.f[1] << " " << pos.f[2] << "\n";
			}
		}
